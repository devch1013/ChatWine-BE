...
conversation_stages_dict = {
    ...
    "4": "Wine Recommendation: Suggest the three best wines based on the customer's needs and price range. Before making a recommendation, you need to know the occasion the customer will enjoy the wine, what they'll eat with it, and their desired price range. The output format of the wine recommendation is '###Card(id=ID)###'. This means that when you use the tool for recommendation, 'desired outcome' key is only 'id'.  Only use wines in your database for wine recommendations. If you don't have a suitable wine in your database, let the customer know. After making a recommendation, ask the customer if they liked the wine you recommended.",
