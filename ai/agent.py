...
conversation_stages_dict = {
    ...
    "3": "Checking Price Range: Asking the customer's preferred price point. Again, remember that the tool for this is not available. But if you know the customer's perferences and price range, then search for the three most suitable wines with tool and recommend wines which output format is '###WineCard(id=ID1,ID2,ID3)###'. This means that when you use the wine database tool for recommendation, 'Desired Outcome' key is only 'id'. If you don't have a suitable wine in your database, let the customer know. After making a recommendation, ask the customer if they liked the wine you recommended.",
    ...
    "4": "Wine Recommendation: Suggest the wines based on the customer's needs and price range. Before making a recommendation, you need to know the occasion the customer will enjoy the wine, what they'll eat with it, and their desired price range. The output format of the wine recommendation is list like '###WineCard(id=ID1,ID2,ID3)###'. This means that when you use the wine database tool for recommendation, the 'Desired Outcome' key must be only 'id'.  Only use wines in your database for wine recommendations. If you don't have a suitable wine in your database, let the customer know. After making a recommendation, ask the customer if they liked the wine you recommended.",
    ...
    "6": "Location Suggestions: Recommend wine bars based on the customer's location and occasion. Before making a recommendation, always use the map tool to find the district of the customer's preferred location. Then use the wine bar database tool to find a suitable wine bar. The output format of the wine bar recommendation is list like '###WineBarCard(id=ID1,ID2,ID3)###', which means that when you use the tool to make a recommendation, the 'Desired Outcome' key must be only 'id'. Only use wine bars in your database for wine bar recommendations. If you don't have a suitable wine bar in your database, let the customer know. After completing the recommendation, ask the customer if they liked the wine bar you recommended",
    ...
    def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        if self.end_reached:
            ...
            if self.last_tokens[-2] == ":":
                ...
                self.queue.append(self.last_tokens[-2])
